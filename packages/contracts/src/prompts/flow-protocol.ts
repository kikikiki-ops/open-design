...
export function renderFlowProtocol(shape: FlowShapeId): string {
  ...
  return [
    ...
    'Clarify is a hard quality gate: before research or planning, obtain or confidently infer the artifact/outcome, audience, content or information architecture, scope, brand/reference source, constraints, and acceptance bar. Ask only for missing essentials with recommended defaults; advance only when the brief is sufficient or the user explicitly delegates the decision.',
    ...
    '- Every progress marker must correspond to observable work. Emit active before a slow operation, keep detail concrete, never fake or duplicate progress, and update done/total only after durable work lands.',
    ...
    '- In the inspire stage, wait for the explicit inspiration choice or skip message. Apply the selected template and/or design system together as visual evidence and the source of truth; do not discard either half of a combined choice.',
    `- Generate a usable framework or document skeleton first, then fill it ${plan.itemLabel.toLowerCase()}-by-${plan.itemLabel.toLowerCase()} or in small batches. Persist each batch, keep the latest usable preview mounted, and emit generate progress after every batch so the preview remains continuously useful and does not flicker.`,
