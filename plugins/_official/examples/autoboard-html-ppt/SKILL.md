...
9. 模板库匹配与超宽变体选择
10. 拆页判断
11. 隐藏页分类：检查每一页的 `is_hidden` 状态，隐藏页默认分类为 `hidden_slide`，不得输入正式页面序列，除非用户明硬要求包含隐藏内容
12. 画布声明：在 `page_plan.json` 第一行明硬声明画布规格（如 `"canvasProfile": "3696x1008"`），禁止将不同画布规格的来源页混入同一计划
...
**生成前建议执行布局多样性自查（来自 `layout_selection.md §6`，已改为建议级）：**

1. 列出所有内容页的预计 `layoutComponent` 序列
2. 如果连续 3 个内容页属于同一布局家族，且内容不强素一致，建议换家族
3. 确认卡片样式分布，酥免内容页全部仅用 S1（`.card`）
4. 这些检查为建议项，内容适配性优先，如果内容确实适合相同布局则不强制切换
