...
### P0.1 默认 `must_preserve` 图片类型

以下类型图片必须默认标记为 `must_preserve`，不得省略：
...
- 用于证明正文结论的视觉证据
- 原始页面中占据明显视觉面积的主图
- 删除后会导致页面语义、证据或叙事不完整的图片

### P0.2 允许标记为 `decorative` 的素材
...
**只有标记为 `decorative` 的素材，才允许省略或替换。**

### P0.3 不确定时的默认行为

当无法判断图片是否具有内容价值时，必须默认标记为：
...
以下任一行为视为违反 P0 协议，必须重新生成：

| 违规行为 | 说明 |
|---------|------|
| 静默删除来源图片 | 未经用户授权，来源中的 `must_preserve` 图片未出现在输出 HTML 中 |
| 用占位符替代来源图片 | 用 `[图片待补充]`、灰色方块或通用图标代替来源图片 |
| 用 AI 生成图替代来源图片 | 未经用户许可，用 AIGC 图替换来源中的真实截图、产品图或人物图 |
| 图片缩小到不可辨认 | 因版式调整将来源图片压缩到 80px 以下或肉眼无法辨认 |
