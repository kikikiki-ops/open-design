...
**使用 `-webkit-line-clamp` 的前提（收窄）**：

1. 组件规格中已声明该区域"最多 N 行"；
2. 审计时 `scrollHeight <= clientHeight`（不超过限制行数）；
3. **被截断的文字必须是"辅助说明"或"补充描述"** — 即该信息缺失后页面核心含义仍然完整；
4. **禁止用于正式内容的任何区域**：
   - 核心数字区（如 `3,580 万/天`、`175% ↑`）
   - 指标名称区（如 "SCVR"、"CVR 前链路"）
   - 卡片主标题（`h2`、`h3` 级别）
   - 案例结论（结果数据行）
   - 列表条目正文（第1行）
   - 图表图例和数据标注
5. **只允许用于**：
   - 卡片底部的"补充说明"（副文字、注释文字）
   - 超过 3 行的长描述性正文（但必须保证第 1–2 行已传达核心信息）
   - 标签组件的补充说明文字

> ⚠️ 关键判断：若截断后用户必须展开才能理解页面意思 → **禁止使用 line-clamp**。若截断后页面含义已完整 → 允许。
