...
### 8.1 HTML 全量资源扫描（P0，生成后必须执行）

生成 `index.html` 后，必须对 HTML 文件执行全量资源路径扫描，验证所有引用资源均可访问：

**扫描范围（缺一不可）：**
```
1. <img src="...">         → 所有图片资源
2. url(...) in style=""    → 内联样式背景图
3. url(...) in <style>     → 嵌入样式表背景图
4. <link href="...">       → CSS/图标资源
5. <script src="...">      → 外部脚本（通常无，但需检查）
```

**禁止资源类型（触发 `resource-path-invalid` 失败）：**
```
❌ 绝对路径：/Users/xxx/...、C:\Users\xxx\...
❌ skill 内部路径：.od-skills/.../assets/...
❌ 非项目目录的绝对路径：/tmp/ppt_extract/...
❌ 临时 URL：包含 token、expires、X-Amz-Expires 等参数的链接
❌ 指向不存在文件的相对路径
```
...
```bash
# 检查是否存在绝对路径引用
grep -n "/Users/\|C:\\\\Users\\\\\|/tmp/\|\.od-skills" index.html && echo "FAIL: absolute path found" || echo "OK"
```

### 8.2 审计等待图片加载（浏览器环境）

在浏览器中执行 `window.__odLayoutAudit()` 前，必须等待所有图片加载完成。在 `__odLayoutAudit` 定义内部的调用入口，加入 image load 等待逻辑（见 §9 审计脚本 `waitForImages` 部分）。

**禁止：** 在 `DOMContentLoaded` 之后立即同步调用 `window.__odLayoutAudit()`，跳过图片加载等待——这会导致图片容器未渲染真实尺寸，`getBoundingClientRect()` 返回错误高度。
