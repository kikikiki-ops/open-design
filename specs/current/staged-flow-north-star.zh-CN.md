...
### 0.1 体验优先级：先建立信心，再展示过程

这是设计垂类链路，优先级不可倒置：

1. **需求充分**：先获得或可靠推断产出形态、目标受众、内容/信息架构、范围、
   品牌或参考、约束与成功标准；关键未知会改变结果时，不能为了显得快而提前生成。
2. **结果好看**：模板与设计系统可组合成为视觉依据，生成前遵循 design taste，
   交付前通过 anti-slop、品牌、自检与精修门禁。
3. **过程可信可见**：brief 锁定后尽快给出第一条真实状态；流式动效、Task Progress
   和 Computer 只展示可观察的工作，不制造假进度或重复播报。
4. **等待可控**：用户可以停留在历史步骤检查细节，不被强制拉回实时；随时可明确
   Jump to live。最新可用预览持续保留，小批量更新，避免闪烁和空白等待。
5. **超出预期但不跑偏**：在完整满足 brief 的基础上增加至多一个有理由的亮点，
   动效服务状态与层级并尊重 reduced motion，以完成度而不是装饰数量制造满足感。
