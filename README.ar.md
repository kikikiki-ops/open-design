...
| **سطح المكتب** | غلاف Electron اختياري بسبيل renderer معزول + sidecar control endpoint (STATUS / EVAL / SCREENSHOT / CONSOLE / CLICK / SHUTDOWN) — يُشغّل `tools-dev inspect desktop screenshot` لاختبارات E2E |
...
| سطح المكتب (اختياري) | غلاف Electron — يكتشف رابط الويب عبر sidecar control endpoint، بدون تخمين منافذ؛ نفس قناة `STATUS`/`EVAL`/`SCREENSHOT`/`CONSOLE`/`CLICK`/`SHUTDOWN` تُشغّل `tools-dev inspect desktop …` لاختبارات E2E |
...
تطبيق سطح المكتب يكتشف رابط الويب تلقائياً عبر sidecar control endpoint — لا حاجة لتخمين المنافذ.
