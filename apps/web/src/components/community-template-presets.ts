...
// Human titles for the remixable community cards that don't ship a bespoke
// starter (kept in sync with COMMUNITY_TEMPLATES in CommunityView.tsx). The
// resolver uses these so every Remix opens a distinct, correctly-named project
// instead of silently remapping onto Electric Studio.
...
// Resolve a community template id to a remixable project preset. Bespoke
// starters win; every other id generates its own distinct project so Remix
// never opens a different template than the card the user clicked.
export function resolveCommunityTemplatePreset(templateId: string): CommunityTemplatePreset {
  const explicit = COMMUNITY_TEMPLATE_PRESETS[templateId];
