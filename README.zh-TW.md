...
| **桌面版** | 可選 Electron 殼：渲染器 sandbox + sidecar control endpoint（STATUS / EVAL / SCREENSHOT / CONSOLE / CLICK / SHUTDOWN）—— 同一通道驅動 `tools-dev inspect desktop screenshot`，跑 E2E |
...
└──────────────┬─────────────────────────────────┬───────────────┘
   ...
   │
   │  可選 sidecar control endpoint：tcp://127.0.0.1:<port>
...
| 桌面版（可選） | Electron 殼 —— 透過 sidecar control endpoint 拿 web URL，不猜埠；同一通道（`STATUS`/`EVAL`/`SCREENSHOT`/`CONSOLE`/`CLICK`/`SHUTDOWN`）驅動 `tools-dev inspect desktop …` 跑 E2E |
...
桌面版透過 sidecar control endpoint 自動探得 web URL —— 不用猜埠。
...
- **Sidecar 協議 + 桌面版自動化。** Daemon、web、desktop 程序都帶型別化的 5 欄位 stamp（`app · mode · namespace · endpoint · source`），並把 JSON-RPC 控制端點暴露在 `tcp://127.0.0.1:<port>`。`tools-dev inspect desktop status \| eval \| screenshot` 就跑在這條通道上，所以 headless E2E 直接打到真實 Electron 殼，不用造定製夾具（[`packages/sidecar-proto/`](packages/sidecar-proto/)、[`apps/desktop/src/main/`](apps/desktop/src/main/)）。
