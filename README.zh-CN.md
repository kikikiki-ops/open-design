...
| **桌面端** | 可选 Electron 壳：渲染器 sandbox + sidecar control endpoint（STATUS / EVAL / SCREENSHOT / CONSOLE / CLICK / SHUTDOWN）—— 同一通道驱动 `tools-dev inspect desktop screenshot`，跑 E2E |
...
└──────────────┬─────────────────────────────────┬───────────────┘
   ...
   │
   │  可选 sidecar control endpoint：tcp://127.0.0.1:<port>
...
| 桌面端（可选） | Electron 壳 —— 通过 sidecar control endpoint 拿 web URL，不猜端口；同一通道（`STATUS`/`EVAL`/`SCREENSHOT`/`CONSOLE`/`CLICK`/`SHUTDOWN`）驱动 `tools-dev inspect desktop …` 跑 E2E |
...
- **Sidecar 协议 + 桌面端自动化。** Daemon、web、desktop 进程都带类型化的 5 字段 stamp（`app · mode · namespace · endpoint · source`），并把 JSON-RPC 控制端点暴露在 `tcp://127.0.0.1:<port>`。`tools-dev inspect desktop status \| eval \| screenshot` 就跑在这条通道上，所以 headless E2E 直接打到真实 Electron 壳，不用造定制夹具（[`packages/sidecar-proto/`](packages/sidecar-proto/)、[`apps/desktop/src/main/`](apps/desktop/src/main/)）。
